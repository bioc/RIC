...
## Adding the experimental desing into the QFeature objects
...
```
As we can see, most peptides uniquely match to one gene. Peptides that match to more than one gene will be excluded in the downstream analysis. 
...
aggregatedWCL<-RIC::agregate_singlepeptides(QWCLpeptidesfiltered_clean,SV_seq, whichorder = c(1,2,4,3,6,5,7,10,9,8), names_samples=  colData(QWCLpeptides)$group )
...
aggregatedRIC<-RIC::agregate_singlepeptides(QRICpeptidesfiltered_clean,SV_seq, whichorder = c(1,4,3,2,5,7,6,9,8,10),  names_samples=  colData(QWCLpeptides)$group )
```
...
## Explore the intensity pattern across replicates
...
```{r calculate-cRIC}
#Make sure things are pass in this order or it will not be correct! This is super breakable code!!! change to names, not positions
...
We can access the output of `test_moderateRIC` to plot a visualize scatterplot Matrices for WCL and RIC experiment using `plot_scatterRIC`.
```{r plot-scatterRIC}
...
Similarly we can access the output of `test_moderateRIC` to plot a visualize volcanoplot  for WCL and RIC experiment using `plot_volcanoRIC`. Note that we only need the first output component of `test_moderateRIC` to represent a volcano. 
