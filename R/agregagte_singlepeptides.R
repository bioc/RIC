...
#' @examples
#' if(interactive()){
#' aggregatedWCL<-agregagte_singlepeptides(QWCLpeptidesfiltered_clean,SV_seq,
...
#' @export
agregagte_singlepeptides <- function (Qfeature,SV_seq, whichorder, names_samples=  paste(sample_names,rep(1:3,each=3),sep='_')) {
  if(is.integer(whichorder)) whichorder <- as.numeric(whichorder)
  assertthat::assert_that(inherits(Qfeature, "QFeatures"),
   ...
   if(ncol(assay(QWCLpeptidesfiltered_clean))+1 !=  length(whichorder) ) {
    stop(paste0("lenght of whichorder is not valid\n",
                "Make sure whichorder has length ",
                ncol(assay(QWCLpeptidesfiltered_clean))+1 ),
         call. = FALSE)
  }
  
  if(ncol(assay(QWCLpeptidesfiltered_clean)) !=  length(names_samples) ) {
    stop(paste0("lenght of names_samples is not valid\n",
                "Make sure names_samples has length ",
                ncol(assay(QWCLpeptidesfiltered_clean))),
         call. = FALSE)
  }
                          
row_peptides <- assay(Qfeature) %>% as.data.frame()%>% rownames_to_column("sequences")
row_peptides[,2:ncol(row_peptides)][row_peptides[,2:ncol(row_peptides)]<1]=NA
...
ProtIDs_who <- mapPeptides(PeptideSet=row_peptides$sequences,
